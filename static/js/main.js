...
/*	   pagination: {
  ...
  var swiper = new Swiper('.moduler-slider', {
    slidesPerView: 3,
    spaceBetween: 30,
    navigation: {
      nextEl: '.swiper-button-next',
...
if (document.querySelector(".log-user")){
let hdbtn=document.querySelector(".log-option");
let show=document.querySelector(".log-user");

show.addEventListener('click',function(e){
	hdbtn.style.visibility="visible";
  e.preventDefault();
})
document.addEventListener("click", function(event) {
  if (!show.contains(event.target)) {
    hdbtn.style.visibility = "hidden";
  }
})
}
// 60 sec countdown
...
countElements.forEach((time, index) => {
  let count = 60;
  const timer = setInterval(() => {
    count--;

    // Color logic (adjust as needed)
    if (count < 16) {
    ...
    if (count === 0) {
      clearInterval(timer);
      expiredElements[index].textContent = "Your Session has Expired";
      sessionElements[index].style.visibility = "hidden";
    }
  }, 1000);

  timers.push(timer); // Store timers for potential cleanup
});

document.addEventListener("DOMContentLoaded", function () {
  const wardrobeIntQuant = document.querySelector('.wardrobe-inputgrp .intQuant');
  const EFTitle = document.querySelector('.EFTitle');

  // Set the initial value
  EFTitle.textContent = `Your requirements for ${wardrobeIntQuant.value} BHK`;

  // Add an event listener to the wardrobe intQuant for changes
  wardrobeIntQuant.addEventListener('input', function () {
      EFTitle.textContent = `Your requirements for ${wardrobeIntQuant.value} BHK`;
  });

  //  code for incrementing/decrementing intQuant values
  const btnn = document.querySelectorAll(".quantBtn");

  btnn.forEach(function (btn) {
      btn.addEventListener("click", function () {
          const parentDiv = btn.closest(".wardrobe-inputgrp");
          const intQuant = parentDiv.querySelector(".intQuant");
          let currentValue = parseInt(intQuant.value);

          if (btn.classList.contains("removebtn")) {
              intQuant.value = Math.max(currentValue - 1, 0);
          } else {
              intQuant.value = Math.min(currentValue + 1, 3);
          }

          // Trigger the input event to update EFTitle
          intQuant.dispatchEvent(new Event('input'));
      });
  });
});
